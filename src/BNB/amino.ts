...
import {UVarInt} from '@helpers/varint'
...
const sortObject = (obj: any): object | null => {
  ...
  const result: any = {}
  sortedKeys.forEach((key: any): void => {
 ...
 * @param num
 */
export const encodeNumber = (num: number): any => UVarInt.encode(num)
 ...
 * @return bytes {Buffer}
 */
export const convertObjectToSignBytes = (obj: object): Buffer =>
 ...
 * @param {Object} obj
 *  */
export const marshalBinary = (obj: object): string => {
 ...
 * @return {Buffer} with bytes length prefixed
 */
export const encodeObjectBinary = (
  obj: any,
  isByteLenPrefix: Boolean,
): Buffer => {
  const bufferArr: any = []
  ...
  Object.keys(obj).forEach((key, index) => {
    ...
    if (is.array(obj[key]) && obj[key].length > 0) {
      bufferArr.push(encodeArrayBinary(index, obj[key], false))
    } else {
...
// Write field key.
const encodeTypeAndField = (index: number, field: any) => {
  index = Number(index)
  const value = ((index + 1) << 3) | typeToTyp3(field)
  return UVarInt.encode(value)
}

const isDefaultValue = (obj: any): Boolean => {
...
const typeToTyp3 = (type: any): number => {
  if (is.boolean(type)) {
    return 0
  }
