...
function VarIntFunc(signed: Boolean) {
  const encodingLength = (n: number): number => {
    if (signed) n *= 2
    if (n < 0) {
  ...
  const encode = (n: number, buffer: any = null, offset: number = 0): any => {
    if (n < 0) {
      throw Error('varint value is out of bounds')
    }
    ...
    let bn: any = new BN(nStr, 10)
    const num255: any = new BN(0xff)
    const num128: any = new BN(0x80)
